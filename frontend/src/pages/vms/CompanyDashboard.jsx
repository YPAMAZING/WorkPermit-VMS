// Company Dashboard - Full dashboard view for company users
// Shows: Recent Visitor Passes, Alerts, Weekly Trend, Pre-approved, etc.
...
const CompanyDashboard = () => {
  ...
  const purposeColors = {
    ...
    Personal: 'bg-gray-100 text-gray-800',
    'Vendor/Client Visit': 'bg-indigo-100 text-indigo-800',
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm text-gray-500 truncate">
                        {gp.visitorCompany || gp.companyFrom} • {gp.hostName || 'Reception'}
                      </p>
                    </div>
                ...
                {overview.purposeBreakdown.map((item, index) => (
                  <div key={index} className="flex items-center justify-between">
                    <span className={`text-xs px-2 py-1 rounded-full ${purposeColors[item.purpose] || purposeColors.OTHER}`}>
                      {item.purpose?.replace('_', ' ') || 'Unknown'}
                    </span>
